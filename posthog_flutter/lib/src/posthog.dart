...
class Posthog {
  ...
  Future<void> captureLog({
    ...
    for (final callback in callbacks) {
      final result = await _applyBeforeSendLog(callback, record);
      if (result == null) {
  ...
  /// Applies a single before-send log callback safely.
  ///
  /// Returns `null` if the record should be dropped, otherwise returns the
  /// (possibly modified) record. A callback that throws is skipped and the
  /// chain continues with the pre-callback record.
  ///
  /// Mirrors the events equivalent `PosthogFlutterIO._applyBeforeSendCallback`;
  /// keep their exception-handling semantics in sync.
  Future<PostHogLogRecord?> _applyBeforeSendLog(
    BeforeSendLogCallback callback,
    PostHogLogRecord record,
  ) async {
    try {
      final result = callback(record);
      if (result is Future<PostHogLogRecord?>) {
        return await result;
      }
      return result;
    } catch (e) {
      debugPrint('[PostHog] beforeSend log callback threw exception: $e');
      return record;
    }
  }
